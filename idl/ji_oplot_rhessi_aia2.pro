;
; overplot some RHESSI data with some AIA images.  Do not bother with the
...
for i = 0, nwchannel - 1 do begin
    ...
    p = plot(time[nonzero], remission_aia[nonzero],$
             name='average AIA emission in RHESSI reconstructed image FOV',$
             xtitle='time (seconds) since ' + initial_time_string[i],$
             ytitle='average emission',$
             title=channel_string)
    plist = LIST(p)
